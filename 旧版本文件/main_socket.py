...
service_ip_and_port = ('162.105.183.67', 23456)
...
class MainWindow(QtWidgets.QMainWindow, ui_main_window.Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.save_dir = None
        self.case_pic_list = None    # 图片路径下的所有图片名称数组
        ...
        print('1')
        self.get_case_pic_list()

    def connect_service(self):
        try:
            self.client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.client.connect(service_ip_and_port)
        except socket.error as msg:
            print(msg)
            sys.exit(1)


    def get_case_pic_list(self):
        # 从服务端获取 case_pic_list

        self.client.send('get_case_pic_list'.encode(encoding='utf-8'))
        list_str_len = int(self.client.recv(1024).decode())
        print(list_str_len)

        recvd_size = 0  # 定义已接收文件的大小
        res = ''
        while not recvd_size == list_str_len: #后续转小于
    ...
    def set_img(self, path=None):
        ...
        self.photo.update()
        self.img_preview_list.setCurrentRow(self.case_pic_list.index(self.img_name))


    def set_3x3_img(self):
        path = os.path.join(self.img_dir, self.img_name) #if path is None else path
        m=Merge(path,self.pic_name_dic,self.img_dir,self.the_shorter_num)
    ...
    def prev_button_listener(self):
        if self.img_dir is not None:
            index = self.case_pic_list.index(self.img_name)

            self.img_name = self.case_pic_list[(index - 1)%len(self.case_pic_list)]
            self.set_img()
    ...
    def open_img_dir_listener(self):
        self.case_pic_list = os.listdir(self.img_dir)
        self.case_pic_list =self.case_pic_list[::-1]
        self.img_name = self.case_pic_list[0]
        self.img_preview_list.clear()
        self.img_preview_list.addItems(self.case_pic_list)
        self.set_img()
        self.pic_name_dic = {ele.split('-')[1]: ele.split('-')[0] for ele in self.case_pic_list}
    ...
    def closeEvent(self, e):
        reply = QtWidgets.QMessageBox.question(self, '确认退出', "是否要退出程序？", QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
