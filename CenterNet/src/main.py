...
def main():
  ...
  for epoch in range(1, opts.num_epochs + 1):
    ...
    saveModelDir = os.path.join(os.path.dirname(__file__), 'weight', 'model_{}.pth'.format(epoch))
    save_model(saveModelDir ,epoch, model, optimizer)
    print('保存成功，去看看')
    with torch.no_grad():
      # trainer.run_epoch('val',epoch, val_loader)
      map,ap50 = test(saveModelDir)

    os.remove(saveModelDir)

    if ap50 > bestAp50:
      newSaveModelDir = "weight/"+str(round(ap50,4))+"-"+str(round(map,4))+"-"+str(epoch)+".pth"
      bestAp50 = ap50
      if lastPt:
        os.remove(lastPt)
      lastPt = newSaveModelDir
      save_model(newSaveModelDir, epoch, model, optimizer)
