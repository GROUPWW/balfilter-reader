...
class in_mysql():
    def __init__(self,case_id_and_model):
        ...
        self.sql = "CREATE TABLE DATABASE_%s (IMG_NAME  CHAR(20) NOT NULL,CONFIDENCE FLOAT,MODEL CHAR(20) )" % (case_id_and_model)
        self.cursor.execute(self.sql)
        print('创建数据库',case_id_and_model,'成功')


    def write_in_database(self,nxn_img_name,conf,used_model):
        self.sql =  "INSERT INTO DATABASE_%s(IMG_NAME,CONFIDENCE,MODEL) VALUES ('%s', %s, '%s')" %(self.case_id_and_model,nxn_img_name,conf,used_model)
        self.cursor.execute(self.sql)
